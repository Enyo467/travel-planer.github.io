...
const MiniDayCard: React.FC<MiniDayCardProps> = ({ day, isSelected, onSelect, onEditName, onRemove, onToggleTime }) => {
    ...
    return (
        <div
            ...
            {...listeners}
        >
            <span className="mini-day-card__name">{day.name}</span>
            ...
            {isMenuOpen && createPortal(
                ...
                >
                    ...
                    <button className="menu-item" onClick={(e) => { e.stopPropagation(); onToggleTime(); setIsMenuOpen(false); }}>{day.showTime === false ? '🕐 Włącz godzinę' : '🕐 Wyłącz godzinę'}</button>
                    <button className="menu-item delete" onClick={(e) => { e.stopPropagation(); if (window.confirm('Czy na pewno usunąć ten dzień?')) onRemove(); setIsMenuOpen(false); }}>🗑️ Usuń dzień</button>
                </div>,
                document.body
            )}
        </div>
